...
| Apple Style 代码块 | 语法高亮、行号、折叠/展开、`fold` 关键字默认折叠、**文件名显示**（`title="filename"`）、**彩色语言徽章**、悬浮动画效果 |
| Mermaid 图表 | flowchart、sequenceDiagram、gantt 等实时渲染 |
| YAML Front Matter | 自动解析文件头元数据，以表格形式展示 |
| 行内元素 | ==高亮==、^上标^、~下标~、`代码` 醒目颜色 |
| 图片处理 | 粘贴截图自动上传（PicGo / NAS 本地 / Base64 三级降级）、悬浮放大效果、点击 Lightbox 全屏查看 |
...
| 代码高亮 & 折叠 | ✅ 完全支持 | Apple Style，`fold` 关键字默认折叠，文件名显示，彩色语言徽章，悬浮动画 |
| 图片 Lightbox | ✅ 完全支持 | 悬浮放大效果，点击图片全屏查看 |
| Mermaid 图表渲染 | ✅ 完全支持 | 纯前端 SVG 渲染 |
| YAML Front Matter | ✅ 完全支持 | 自动解析元数据表格 |
| ==高亮== / ^上标^ / ~下标~ | ✅ 完全支持 | markdown-it 插件 |
| 导出 .docx / .html / .pdf | ✅ 完全支持 | 纯前端生成，直接下载 |
| 图片粘贴保存 | ✅ 支持 | PicGo → NAS 本地保存 → Base64 三级降级，全屏上传动画提示 |
| PicGo 上传 | ✅ 支持 | 默认 `127.0.0.1:36677`，兼容 PicGo 新旧版本（`files` / `list[]`） |
